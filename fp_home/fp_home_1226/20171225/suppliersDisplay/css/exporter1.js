		...
		fpbase.Ajax.Read("", "export/company", "getCompanyProfileImg", {
			...
			if (resultData != null && resultData.length > 0) {
				for (var i = 0; i < resultData.length; i++) {
					...
					arrImg.push(getUrlByServerLocationFlag(resultData[i].imgUrlCn,resultData[i].imgUrlEn));
				}
				section1_Img(arrImg);
			}
		});
...
//		}
		var url = getUrlByServerLocationFlag(item.firmLogoUrlCn,item.firmLogoUrlEn);
		if(url != ""){
			profileInfo["logoImageUrl"] = url+"&width=80";
		}
	...
	var makeOneFruit = function(friutImgData, description) {
		...
		for (var i = 0; i < friutImgData.length; i++) {
			...
			imgHtml += "    <img src='" + getUrlByServerLocationFlag(friutImgData[i].imageUrlCn,friutImgData[i].imageUrlEn) + "' alt='' longdesc='https://www.freshport.com' width='100px'>";
			imgHtml += "  </div>";
			...
			imgHtml += "   <img src='" + getUrlByServerLocationFlag(friutImgData[i].imageUrlCn,friutImgData[i].imageUrlEn) + "' alt='' longdesc='https://www.freshport.com' width='300px'>";
			imgHtml += "  </div>";
			imgHtml += "</div>";
			index = index - 1;
		}
...
//		}
		var url = getUrlByServerLocationFlag(item.imgUrlCn,item.imgUrlEn);
		if(url != ""){
			team["url"] = url+"&width=250";
		}
	...
	var setion1 = function(company) {
		...
		if (company.file.length) {
			...
			for (var i = 0; i < company.file.length; i++) {
				var fileUrl = "";
				if(serverLocationFlag == "1"){// 1锛氬浗鍐� 
					fileUrl= company.file[i]["fileUrlCn"];
				}else if(serverLocationFlag == "2"){ // 2锛氬浗澶�
					fileUrl= company.file[i]["fileUrlEn"];
				}
				fileHtml += '<a href="' + fileUrl + '" target="_blank" class="profile" alt="Company Brochure">' + company.file[i]["fileName"] + '</a>';
			}
			fileHtml += "</div>";
	...
	var section1_Img = function(arrImg) {
		...
		var showproduct = {
			"boxid" : "showbox",
			"sumid" : "showsum",
			"boxw" : 300, // 鐎硅棄瀹�,鐠囥儳澧楅張顑胯厬鐠囬攱濡哥€逛粙鐝繅顐㈠晸閹存劒绔撮弽锟�
			"boxh" : 300, // 妤傛ê瀹�,鐠囥儳澧楅張顑胯厬鐠囬攱濡哥€逛粙鐝繅顐㈠晸閹存劒绔撮弽锟�
			"sumw" : 58, // 閸掓銆冨В蹇庨嚋鐎硅棄瀹�,鐠囥儳澧楅張顑胯厬鐠囬攱濡哥€逛粙鐝繅顐㈠晸閹存劒绔撮弽锟�
			"sumh" : 58, // 閸掓銆冨В蹇庨嚋妤傛ê瀹�,鐠囥儳澧楅張顑胯厬鐠囬攱濡哥€逛粙鐝繅顐㈠晸閹存劒绔撮弽锟�
			"sumi" : 7, // 閸掓銆冮梻鎾
			"sums" : showlen, // 閸掓銆冮弰鍓с仛娑擃亝鏆�
			"sumsel" : "sel",
			"sumborder" : 1, // 閸掓銆冩潏瑙勵攱閿涘本鐥呴張澶庣珶濡楀棗锝為崘锟�0閿涘矁绔熷鍡楁躬css娑擃厺鎱ㄩ弨锟�
			"lastid" : "showlast",
			"nextid" : "shownext"
		}; // 閸欏倹鏆熺€规矮绠�
		$.ljsGlasses.pcGlasses(showproduct); // 閺傝纭剁拫鍐暏閿涘苯濮熻箛鍛躬閸旂姾娴囩€瑰苯鎮楅幍褑顢�
	}
	...
	var drawSeason = function(seasoneData) {
		...
		var count = seasoneData.length > 10 ? 10 : seasoneData.length;// 娴肩姳姹夋潻鍥ㄦ降閻ㄥ嫭鏆熼幑锟�
		// 閸斻劍鈧胶鏁撻幋锟� 閺傝鐗搁懗灞炬珯
		var $line = $('#line');
		var $temp = $('<div></div>');
	...
	var section4Video = function(linkArr) {
		...
		if( i == 0 ){
		var $li = '<li class="active"  index="'+i+'"  ></li>';
		}else{
	...
	$prev.click(function(){
		...
		$(this).add( $next ).removeClass("active");
	...
	$next.click(function(){
		...
		current = index + 1;
		$(this).add( $prev ).removeClass("active").
		$active = $allLi .removeClass("active").eq(current).addClass("active");
		$js_videoCon_1.attr({"src":linkArr[current]}); 
	});
