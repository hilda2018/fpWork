...
public class JwtTestCase {
	...
	public void test1() {
		...
		Date date = new Date();
		payload.put("uid", "291969451");// 用户id
		payload.put("iat", date.getTime());// 生成时间:当前
		payload.put("ext", date.getTime() + 1000 * 60 * 60 * 30L);// 过期时间2小时
		...
		System.out.println("从token中取出的payload数据是：" +dataobj.get("uid"));
	...
	public void test2() {
		...
		payload.put("ext", date.getTime()+1000*60);// 过期时间就是当前
		token = Jwt.createToken(payload);
		System.out.println("新生成的token是：" + token+"\n马上将该token进行校验");
		Map<String, Object> resultMap = Jwt.validToken(token);
		System.out.println("校验结果是:" + getResult((String)resultMap.get("state")) );
	...
	public void test4() {
		// 校验异常的情况----------------------------------------------------------------------------------------------------
		String token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1aWQiOiI2N2ZjYTUzZS1hYzIzLTQ3MTgtOTdmNC1kZWUxYTA1NzA0M2IiLCJwYXJlbnRpZCI6ImJlZGRjNzI0LWU5MWEtNGJmNy1iOGY0LWJkNTI4MzVlN2RmYyIsImlhdCI6MTQ3NDk3MDk2MjkxMywiZXh0IjoxNDczMjY3OTk1NjE3fQ.ZG-Be7uVV5vvgi2Wxp7rCZaW8-f3EZViFxL9q3CDZH0";
