...
import com.nova.frame.base.Validate;
	...
	*/
	public class ValidateUtil {
	     ...
	     * 只能输英文 数字 中文 以及空格
	     */
	    public static final String STR_ENG_CHA_NUM_SPACE = "^[a-zA-Z0-9\u4e00-\u9fa5\\s]+$";
	    /**
	     *
	     */
	    /***
...
/*	    public static final String DATE_ALL="((^((1[8-9]\\d{2})|([2-9]\\d{3}))([-\\\\\._]?)(10|12|0?[13578])([-\\\\\._]?)(3[01]|[12][0-9]|0?[1-9])$)" +
	    ...
	    /**
	     * URL正则表达式
	      * 匹配 http www ftp  正则: http(s)?://([\\w-]+\\.)+[\\w-]+(/[\\w- ./?%&=]*)?
	     */
	    public static final String URL = "^(http|www|ftp|https)?(:\\)?(\\w+(-\\w+)*)(\\.(\\w+(-\\w+)*))*((:\\d+)?)(/(\\w+(-\\w+)*))*(\\.?(\\w)*)(\\?)?" +
	                                    "(((\\w*%)*(\\w*\\?)*(\\w*:)*(\\w*\\+)*(\\w*\\.)*(\\w*&)*(\\w*-)*(\\w*=)*(\\w*%)*(\\w*\\?)*" +
	                                    "(\\w*:)*(\\w*\\+)*(\\w*\\.)*" +
	                                    "(\\w*&)*(\\w*-)*(\\w*=)*)*(\\w*)*)$" ;
	    /**
	    ...
	    public static final String IDCARD="((11|12|13|14|15|21|22|23|31|32|33|34|35|36|37|41|42|43|44|45|46|50|51|52|53|54|61|62|63|64|65)[0-9]{4})" +
	       ...
	       /**
	        * 2.匹配科学计数 e或者E必须出现有且只有一次 结尾包含 Dd
	        ...
	        * 小数点后最多保留2位小数
	        *////^([1-9]\d*|0)(\.\d{1,2})?$/
	       public final static String DECIMAL2="^([1-9]\\d*|0)(\\.\\d{1,2})?$";
	//------------------验证方法
	    /**
	     * 判断字段是否为空 符合返回 ture
	     * @param str
	    ...
	    /**
	     * 判断字段是否为数字 正负整数 正负浮点数 符合返回 ture
	     * @param str
	     * @return boolean
	     */
	    ...
	    public static boolean isDecimal2(String str){
	    	return Regular(str, DECIMAL2);
	    	
	    }
	    /**
	     * 判断字段是否为负浮点数正则表达式 <=0 符合返回 ture
	     * @param str
	     * @return boolean
	     */
...
/*	    public static  boolean isDate(String str) {
	    ...
	    /**
	     * 判断字段是否超长
	     * 字串为空返回 false, 超过长度{leng}返回ture 反之返回false
	     * @param str
	     * @param leng
	     * @return boolean
	     */
	     ...
	     * @return
	     */
	   public static boolean isSTR_ENG_CHA_NUM(String str){
	    ...
	    * @return
	    */
	   public static boolean isSTR_ENG_CHA_NUM_SPACE(String str){
		   return Regular(str, STR_ENG_CHA_NUM_SPACE);
	   }
	    
	    public static void main(String[] args) {	
	    	System.out.println(ValidateUtil.isSTR_ENG_CHA_NUM_SPACE(" 1.1"));
	        }
	}
